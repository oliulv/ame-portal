...
export const list = query({
  ...
  handler: async (ctx, args) => {
    ...
    templates.sort((a, b) => (a.sortOrder ?? Infinity) - (b.sortOrder ?? Infinity))

    // Deduplicate "Join AccelerateMe" per cohort
    const seenAccelerateMe = new Set<string>()
    return templates.filter((t) => {
      const isAMe =
        t.title === 'Join AccelerateMe' || t.title?.toLowerCase().includes('join accelerateme')
      if (isAMe) {
    ...
    if (args.isActive) {
      ...
      for (const startup of startups) {
        ...
        const existingGoal = await ctx.db
          .query('startupGoals')
          .withIndex('by_goalTemplateId', (q) => q.eq('goalTemplateId', templateId))
