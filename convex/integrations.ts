...
export const connectStripe = action({
  ...
  handler: async (ctx, args) => {
    ...
    const accountName =
      account.business_profile?.name || account.email || "Stripe Account";

    // Get founder's startup IDs
    const founderProfiles = await ctx.runQuery(
      internal.integrations.getFounderStartupId
    );
    ...
    await ctx.runMutation(internal.integrations.storeStripeConnectionInternal, {
...
/**
 * Internal mutation for storing Stripe connection (used by action).
 */
export const storeStripeConnectionInternal = mutation({
  args: {
    startupId: v.id("startups"),
    accessToken: v.string(),
    accountId: v.string(),
